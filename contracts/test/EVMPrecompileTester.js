...
describe("EVM Test", function () {
  describe("EVM Precompile Tester", function () {
    describe("EVM Bank Precompile Tester", function () {
      ...
      before(async function () {
        ...
        const contractABIPath = path.join(
          __dirname,
          "../../precompiles/common/erc20_abi.json"
        );
      ...
      it("No Approve and TransferFrom fails", async function () {
        ...
        await expectRevert.unspecified(
          erc20AsOwner2.transferFrom(owner, receiver, 100)
        );
      });
      ...
      it("Approve and TransferFrom functions", async function () {
        ...
        const transferFromTx = await erc20AsOwner2.transferFrom(
          owner,
          owner2,
          100
        );
    ...
    describe("EVM Oracle Precompile Tester", function () {
      const OraclePrecompileContract =
        "0x0000000000000000000000000000000000001008";
      before(async function () {
        const exchangeRatesContent = readDeploymentOutput(
          "oracle_exchange_rates.json"
        );
        const twapsContent = readDeploymentOutput("oracle_twaps.json");

        exchangeRatesJSON =
          JSON.parse(exchangeRatesContent).denom_oracle_exchange_rate_pairs;
        ...
        const contractABIPath = path.join(
          __dirname,
          "../../precompiles/oracle/abi.json"
        );
        const contractABI = require(contractABIPath);
        // Get a contract instance
        oracle = new ethers.Contract(
          OraclePrecompileContract,
          contractABI,
          signer
        );
      });
      ...
      it("Oracle Exchange Rates", async function () {
        ...
        for (let i = 0; i < exchangeRatesLen; i++) {
          ...
          expect(exchangeRates[i].oracleExchangeRateVal.exchangeRate).to.be.a(
            "string"
          ).and.to.not.be.empty;
          expect(exchangeRates[i].oracleExchangeRateVal.exchangeRate).to.be.a(
            "string"
          ...
          expect(
            exchangeRates[i].oracleExchangeRateVal.lastUpdateTimestamp
          ).to.exist.and.to.be.gt(0);
        }
      });
    ...
    describe("EVM Wasm Precompile Tester", function () {
      ...
      before(async function () {
        ...
        const contractABIPath = path.join(
          __dirname,
          "../../precompiles/wasmd/abi.json"
        );
        const contractABI = require(contractABIPath);
        // Get a contract instance
        wasmd = new ethers.Contract(
      ...
      it("Wasm Precompile Instantiate", async function () {
        ...
        instantiate = await wasmd.instantiate(
          wasmCodeID,
          "",
          instantiateBz,
          "counter-contract",
